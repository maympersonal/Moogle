...
{	
	...
	private Sinonimos BusquedaSinonimos;//sinónimos de las palabras
	...
	{	List<string> archivos;//listado de los documentos
    	...
    	Utiles.CaminoSinonimos = directorio + "sinonimos.txt";//inicializamos el camino del archivo con los sinónimos
		...
		BusquedaSinonimos = new Sinonimos(Utiles.CaminoSinonimos);//inicializamos la búsqueda de sinónimos
    ...
    {              
        ...
        if ((palabra != null) && (Palabras != null) && (palabra.Length !=0) && (Palabras.Count !=0)){//si se cumplen las condiciones
            foreach(string temppalabra in Palabras) {//pala cada una de las palabras en la lista de palabras de los documenton
    ...
    {   double[] arrDocumentosIDFVector;//arreglo para el vector IDF de los documentos
        ...
        //calculo del vector IDF de los documentos
	 	for(int i = 0; i< CantidadTerminos; i++ ){//para cada uno de los terminos de la consulta
            int frecuencia=0;//inicializamos la frecuencia
            for(int j = 0; j< CantidadDocumentos; j++ ){//para cada uno de los documentos
                if(arrDocumentosTFVector[j,i]>0)//si el termino aparece en el documento
                    frecuencia++;//icrementamos la frecuencia
                
               }
           arrDocumentosIDFVector[i]  = Math.Log10((double)CantidadDocumentos/(double)frecuencia)+1; //calculamos el valor de IDF del termino en el documento
        }
        //Calculo de la matriz TF-IDF	
        for(int i = 0; i< CantidadDocumentos; i++ ){//para cada documento
            for(int j = 0; j< CantidadTerminos; j++ )//para cada uno de los terminos
		...
		for(int i = 0; i< CantidadDocumentos; i++ ){//para cada documento
			...
			querydenominator=0;
			for(int j = 0; j< CantidadTerminos; j++ ){//para cada uno de los terminos
           		numerator=numerator+((float)arrDocumentosTFIDFVector[i,j]*(float)arrConsultaTFVector[j]);//sumamos valores
                denominator= denominator+Math.Pow((double)arrDocumentosTFIDFVector[i,j],2);
				querydenominator = querydenominator+Math.Pow((double)arrConsultaTFVector[j],2);
			}	
	...
	{	
		...
		//paso 3
		for ( int i = 0; i < estadotermino.Length; i++){//para cada termino
			if (estadotermino[i] == 3) {//si esta marcado como paso 3
				listasinonimos = BusquedaSinonimos.BuscarSinonimos(fraseconsulta[i]);//buscamos los sinónimos de la palabra
				
				if (listasinonimos.Count>0){//si hay sinonimos
					sumacolumna =0;//inicializamos la suma
					j = 0;
					foreach(var doc in DocumentosConsulta ){//para cada documento
						foreach(var sino in listasinonimos ){//para cada sinónimo

							vectoresdocumentos[j,i] = vectoresdocumentos[j,i]+(doc.CuentaTermino(sino)*3);//ponemos las veces que aparece multiplicada por tres para que el peso sea menor que la palabra y la raiz de esta
						}
					...
					if (sumacolumna == 0)	//si la suma de la colunma sigue siendo 0
						estadotermino[i] = 4;	//lo marcamos como paso 4
				}
				else
					estadotermino[i] = 4;//lo marcamos como paso 4
