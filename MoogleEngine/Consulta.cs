...
{	private string strTexto; //Texto original de la consulta
	...
	private List<Utiles.ParesPalabras> lTerminosCercanos = new List<Utiles.ParesPalabras>();//Lista de pares de terminos que se le calcula la distancia entre ellos
 	...
 	public Dictionary<string,int> TerminosImportantes
	{
 	...
 	public List<Utiles.ParesPalabras> TerminosCercanos
	{
	...
	{   List<string> palabras = new List<string>();//Lista temporal de palabras
		...
		string ultimotermino = String.Empty;//ultimo termino procesado
	 	...
	 	palabras = Utiles.SeparaPalabras(strTexto, Utiles.OperadoresBusqueda); //Se crea una lista de las palabra que contiene lla consulta respetando los operadores de busqueda
	 	foreach( string palabra in palabras ){//para cada palabra de la lista
			switch (palabra[0]){//se separan los casos por el primer caracter de la palabra
        		case '!'://caso de termino prohibido
            		...
            		if (!Utiles.EsPalabraVacia(temp)){//Si no es palabra vacía
						temp = Utiles.NormalizarPalabra(temp);//Se normaliza la palabra
        		...
        		default://procesamiento de los otros terminos que no son obligatorios, importantes o prohibidos
            		...
            		if (!Utiles.EsPalabraVacia(palabra)) {//Si no es palabra vacía
						temp = Utiles.NormalizarPalabra(palabra);//Se normaliza la palabra
						if (distanciaentrepalabras) {//si estamos procesando un par de palabras que se pide la distancia
							lTerminosCercanos.Add(new Utiles.ParesPalabras(ultimotermino,temp));//se agrega  a los terminos cercanos el par de palabras ultimotermino y el termino actual
